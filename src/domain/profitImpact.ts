...
export type OpportunityEvidence =
  | {
      ...
      thresholdPct: number;
      topRefundedOrderIds?: Array<number | string>;
    }
  | {
      // HIGH_FEES
      feeRatePct: number;
      thresholdPct: number;
      topHighFeeOrderIds?: Array<number | string>;
    }
  | {
      // LOW_MARGIN
      contributionMarginPct: number;
      thresholdPct: number;
      topLowMarginOrderIds?: Array<number | string>;
      basis: "BEFORE_ADS" | "AFTER_ADS";
    }
  | {
      // NEGATIVE_CM
      negativeOrdersCount: number;
      exampleNegativeOrderIds?: Array<number | string>;
      basis: "BEFORE_ADS" | "AFTER_ADS";
    }
  | {
      // MISSING_COGS
      missingCogsOrdersCount: number;
      exampleMissingCogsOrderIds?: Array<number | string>;
    };
...
export type Opportunity = {
  ...
  estimatedLoss: number; // for selected period
...
function enrichWithTimeScaling(lossForPeriod: number, days: number) {
...
function topIds<T extends { id?: number | string }>(rows: T[], limit = 3) {
  return rows
    .map((r) => r.id)
...
function actionsForReason(reason: Reason): string[] {
  switch (reason) {
    case "HIGH_REFUNDS":
      return [
...
function detectBasis(orders: OrderForImpact[]): "BEFORE_ADS" | "AFTER_ADS" {
...
function selectedProfit(o: OrderForImpact, basis: "BEFORE_ADS" | "AFTER_ADS") {
  if (basis === "AFTER_ADS") {
...
function selectedProfitPct(o: OrderForImpact, basis: "BEFORE_ADS" | "AFTER_ADS") {
...
export function buildProfitImpact(params: {
  ...
  totals: {
    ...
    // ✅ Optional totals after ads (if you have it, pass it; else we compute from orders)
    profitAfterAds?: number;
  };
}) {
