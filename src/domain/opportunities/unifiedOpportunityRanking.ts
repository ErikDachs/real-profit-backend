...
function mkWhyEvidence(input: {
  ...
  // Inputs we may have depending on type
...
function mk(
  params: Omit<
    UnifiedOpportunity,
    "estimatedMonthlyLoss" | "score" | "confidence" | "controllability" | "severity"
  > & {
    lossInPeriod: number;
  }
): UnifiedOpportunity {
...
function nonZero(opps: UnifiedOpportunity[]): UnifiedOpportunity[] {
...
export type UnifiedOpportunityRankingParams = {
  ...
  // 1) ProfitImpact AFTER_ADS signals (already deterministic in your system)
  profitImpact?: {
    lowMargin?: { lossInPeriod: number; marginPct?: number | null };
    negativeCm?: { lossInPeriod: number; cm?: number | null; cmPct?: number | null };
  };

  // 2) Refund totals (already computed in summary)
  refunds?: {
    lossInPeriod: number; // e.g. total refunds value in period
    refundRatePct?: number | null; // optional, for meta/UI
  };

  // 3) Fees totals (already computed in summary)
  fees?: {
    lossInPeriod: number; // total payment fees in period
    feePctOfNet?: number | null; // optional
  };

  // 4) Missing COGS count (already tracked)
  missingCogsCount?: number;
  missingCogsLossInPeriod?: number;

  // 5) Shipping subsidy insight (already exists in your system)
  shippingSubsidy?: {
    lossInPeriod: number;
    subsidyRatePct?: number | null;
  };

  // Margin drift (computed from daily profits)
  marginDrift?: {
    lossInPeriod: number;
  ...
  // Break-even risk (computed from ad intelligence)
  breakEvenRisk?: {
    lossInPeriod: number;
    adSpend?: number | null;
    currentRoas?: number | null;
  ...
  // Fixed costs (allocated into period by SSOT allocator)
  fixedCosts?: {
    lossInPeriod: number;
    fixedCostRatePct?: number | null;
  };
