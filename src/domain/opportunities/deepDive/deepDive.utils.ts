...
export function lossInPeriodFromMonthly(params: { estimatedMonthlyLoss: number; days: number }) {
...
export function finalizeShares(drivers: DeepDiveDriver[], totalImpact: number) {
...
export function concentration(drivers: DeepDiveDriver[]) {
  const top = [...drivers].sort((a, b) => Number(b.impact || 0) - Number(a.impact || 0));
  const total = top.reduce((s, d) => s + Number(d.impact || 0), 0);

  const sumTop = (n: number) => top.slice(0, n).reduce((s, d) => s + Number(d.impact || 0), 0);
...
export function findOpportunity(opps: UnifiedOpportunity[], type: OpportunityType) {
