...
function scenarioToOverrides(params: { scenario: string; baseCostProfile: any }) {
...
export function registerProfitScenariosRoute(app: FastifyInstance, ctx: ShopifyCtx) {
    ...
    try {
      ...
      if (!scenarioOverrides) {
        return reply.status(400).send({
          ...
          details:
            "Valid: fees_-10 | fees_-20 | fees_-30 | ship_-25 | ship_-50 | ship_-75 | ship_off",
        });
      }
