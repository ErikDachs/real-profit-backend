...
export async function exchangeCodeForAccessToken(params) {
    ...
    if (!res.ok) {
        throw new ShopifyOAuthError(`Token exchange failed (${res.status}): ${text}`, "TOKEN_EXCHANGE_FAILED", res.status);
    }
    let json;
    try {
    ...
    if (!token)
        throw new ShopifyOAuthError("Missing access_token in response", "TOKEN_EXCHANGE_FAILED", 502);
