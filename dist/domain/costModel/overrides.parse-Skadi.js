 ...
 */
export function costOverridesFromAny(input) {
    ...
    const shippingCost = input.shippingCostPerOrder ?? input.defaultShippingCost ?? input.shippingCost ?? undefined;
    const includeShippingCost = input.includeShippingCost ?? undefined;
    // ✅ NEW: gift cards flag (flat or nested flags)
    const flagsObj = input.flags && typeof input.flags === "object" ? input.flags : undefined;
    const excludeGiftCards = flagsObj?.excludeGiftCards ?? input.excludeGiftCards ?? undefined;
    ...
    if (feePercent !== undefined || feeFixed !== undefined) {
        ...
        if (feePercent !== undefined)
            out.payment.feePercent = Number(feePercent);
        if (feeFixed !== undefined)
            out.payment.feeFixed = Number(feeFixed);
    }
    ...
    if (includeShippingCost !== undefined || excludeGiftCards !== undefined) {
        out.flags = {};
        if (includeShippingCost !== undefined)
            out.flags.includeShippingCost = Boolean(includeShippingCost);
        if (excludeGiftCards !== undefined)
            out.flags.excludeGiftCards = Boolean(excludeGiftCards);
    }
    ...
    if (fixedCostsAllocationMode !== undefined ||
        ...
        out.fixedCosts = {};
        if (fixedCostsAllocationMode !== undefined)
            out.fixedCosts.allocationMode = String(fixedCostsAllocationMode);
        if (fixedCostsDaysInMonth !== undefined)
            out.fixedCosts.daysInMonth = Number(fixedCostsDaysInMonth);
        if (fixedCostsMonthlyItems !== undefined) {
            // accept array only, ignore otherwise
            if (Array.isArray(fixedCostsMonthlyItems))
                out.fixedCosts.monthlyItems = fixedCostsMonthlyItems;
        }
    }
