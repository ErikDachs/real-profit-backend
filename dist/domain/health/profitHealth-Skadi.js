...
import { getDefaultProfitHealthConfig } from "./healthConfig";
...
export function computeProfitHealthFromSummary(params) {
    ...
    const ordersCountRaw = params.ordersCount !== undefined ? params.ordersCount : params.count !== undefined ? params.count : 0;
    ...
    if (adSpend !== undefined && Number.isFinite(adSpend) && adSpend > 0) {
        ...
        if (breakEvenRoas !== null &&
            ...
            const ratio = (salesRoas ?? null) !== null && breakEvenRoas ? salesRoas / breakEvenRoas : null;
            if (ratio !== null && Number.isFinite(ratio)) {
                if (ratio >= cfg.adEfficiency.greatMultiplier)
                    adScore = 100;
                else if (ratio >= cfg.adEfficiency.goodMultiplier) {
                    const k = (ratio - cfg.adEfficiency.goodMultiplier) /
                    ...
                    const k = (ratio - cfg.adEfficiency.okMultiplier) /
    ...
    // --- Shipping subsidy ---
    let shippingScore;
    let shippingSubsidyLoss = null;
    let shippingSubsidyPct = null;
    const shippingRevenue = params.shippingRevenue;
    const shippingCost = params.shippingCost;
    if (shippingRevenue !== undefined &&
    ...
    // --- Missing COGS ---
    // ✅ SSOT rule: if upstream (OrdersSummary) provides missingCogsRatePct, Health must NOT recompute it.
    let missingCogsScore;
    let missingCogsRatePct = null;
    const rateFromUpstream = params.missingCogsRatePct;
    if (rateFromUpstream !== undefined && rateFromUpstream !== null && Number.isFinite(Number(rateFromUpstream))) {
        missingCogsRatePct = Number(rateFromUpstream);
        missingCogsScore = scoreLowerIsBetter(missingCogsRatePct, cfg.missingCogsRatePct);
    }
    else if (params.missingCogsCount !== undefined && Number.isFinite(params.missingCogsCount) && ordersCount > 0) {
        missingCogsRatePct = (Number(params.missingCogsCount) / ordersCount) * 100;
        missingCogsScore = scoreLowerIsBetter(missingCogsRatePct, cfg.missingCogsRatePct);
    }
    // --- Fixed costs pressure ---
    let fixedCostScore;
    let fixedCostRatePct = null;
    const fixedCostsAllocatedInPeriod = params.fixedCostsAllocatedInPeriod;
    if (fixedCostsAllocatedInPeriod !== undefined &&
    ...
    if (adScore === undefined)
        baseWeights.adEfficiency = 0;
    if (shippingScore === undefined)
        baseWeights.shippingSubsidy = 0;
    if (missingCogsScore === undefined)
        baseWeights.missingCogs = 0;
    if (fixedCostScore === undefined)
        baseWeights.fixedCostPressure = 0;
    const w = normalizeWeights(baseWeights);
    const scoreRaw = cmScore * w.contributionMarginPct +
...
export function computeProfitHealthFromOrdersSummary(summary, config) {
    return computeProfitHealthFromSummary({
        ...
        missingCogsRatePct: summary.missingCogsRatePct ?? null,
