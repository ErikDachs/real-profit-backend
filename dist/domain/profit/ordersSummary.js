...
export async function buildOrdersSummary(params) {
    ...
    const rawGrossSalesTotal = orders.reduce((sum, o) => sum + Number(o.total_price || 0), 0);
    const rawRefundsTotal = orders.reduce((sum, o) => sum + extractRefundsFromOrder(o), 0);
    ...
    const shippingRevenue = orders.reduce((sum, o) => sum + extractShippingRevenueFromOrder(o), 0);
    ...
    const nonGiftCardOrderCount = orders.reduce((n, o) => n + (orderIsGiftCardOnly(o) ? 0 : 1), 0);
    ...
    const unitCostByVariant = params.unitCostByVariant ?? (await cogsService.computeUnitCostsByVariant(shopifyGET, variantIds));
    ...
    const fixedCostPerOrder = fixedAllocMode === "PER_ORDER" && count > 0 ? round2(fixedCostsAllocatedInPeriod / count) : 0;
