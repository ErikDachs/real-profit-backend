...
import { DEFAULTS, buildActions, periodLabel } from "./utils.js";
...
export function buildProfitKillersInsights(params) {
    ...
    const worstProducts = pickWorstProducts(products, limit);
    const bestProducts = pickBestProducts(products, limit);
    ...
    const legacyOpportunities = buildProfitImpact({
        ...
        orders: ordersWithReasons.map((o) => ({
            ...
            profitAfterAds: o.profitAfterAds,
            reasons: o.reasons,
        })),
        totals: {
            grossSales: totals.grossSales,
            ...
            contributionMargin: totals.contributionMargin,
            contributionMarginPct,
            profitAfterAds: ordersWithReasons.reduce((s, o) => s + Number(o.profitAfterAds ?? 0), 0),
        },
    });
    ...
    const breakEvenRisk = adIntelligence
        ...
        : null;
    if (breakEvenRisk)
        insights.push(breakEvenRisk);
    // -------------------------
    // STRICT Profit Killers ranking inputs (Option A)
    // -------------------------
    const shippingLossInPeriod = shippingSubsidy && Number.isFinite(Number(shippingSubsidy.totalShippingImpact))
        ? Math.max(0, -Number(shippingSubsidy.totalShippingImpact))
        : shippingSubsidy && Number.isFinite(Number(shippingSubsidy.estimatedLossInPeriod))
            ? Math.max(0, Number(shippingSubsidy.estimatedLossInPeriod))
            : 0;
    const legacyAll = (legacyOpportunities?.all ?? []);
    const missingCogsOpp = legacyAll.find((x) => x?.reason === "MISSING_COGS");
