...
function buildRefundDriver(params) {
    ...
    const haveRefundInputs = Number.isFinite(sGross) &&
...
function maybeBuildFixedCostsDriver(params) {
    ...
    const haveFixed = (s.fixedCostsAllocatedInPeriod !== undefined && Number.isFinite(sFixed)) ||
...
export function decomposeMarginDriftCore(params) {
    ...
    const shortProfit = params.mode === "OPERATING" ? shortCm - shortShip - shortFixed : shortCm;
    const longProfit = params.mode === "OPERATING" ? longCm - longShip - longFixed : longCm;
    ...
    if (params.mode === "CM") {
        ...
        if (refundDriver)
            drivers.push(refundDriver);
    }
    // COGS
    drivers.push(buildRatioDriver({
        code: "COGS",
