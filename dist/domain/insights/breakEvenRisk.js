...
export function computeBreakEvenRisk(params) {
    ...
    if (!status || status === "INSUFFICIENT_INPUT") {
        ...
        if (!(Number.isFinite(Number(spend)) && Number.isFinite(Number(roas)) && Number.isFinite(Number(be))))
            return null;
    }
    const roasGap = Number.isFinite(Number(roas)) && Number.isFinite(Number(be)) ? round2(Number(roas) - Number(be)) : null;
    const leak = status === "BURNING_CASH"
