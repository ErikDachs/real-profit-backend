...
export function buildUnifiedRankingInputs(params) {
    const { totals, missingCogsCount, missingCogsLossInPeriod, legacyAll, shippingSubsidy, shippingLossInPeriod, marginDrift, breakEvenRisk, fixedCosts } = params;
    ...
    const lowMarginOpp = legacyAll.find((x) => x?.reason === "LOW_MARGIN");
    const negativeOpp = legacyAll.find((x) => x?.reason === "NEGATIVE_CM");
    ...
    const fixedCostRatePctTotal = totals.netAfterRefunds > 0 ? (fixedCostsAllocatedInPeriod / totals.netAfterRefunds) * 100 : 0;
    ...
    return {
        ...
        shippingSubsidy: shippingLossInPeriod > 0
            ? {
                lossInPeriod: shippingLossInPeriod,
                subsidyRatePct: shippingSubsidy?.subsidyRatePct ?? null,
            }
            : undefined,
        marginDrift: marginDrift && marginDrift.status === "DETERIORATING"
            ? {
            ...
            : undefined,
        breakEvenRisk: breakEvenRisk && breakEvenRisk.status === "BURNING_CASH"
            ? {
