...
function shippingImpactValue(o) {
    const impactRaw = o.shippingImpact;
...
export function pickWorstOrders(params) {
    const { orders, type, limit } = params;
    const sorted = [...orders].sort((a, b) => {
        if (type === "HIGH_REFUNDS")
            return Number(b.refunds || 0) - Number(a.refunds || 0);
        if (type === "HIGH_FEES")
            return Number(b.paymentFees || 0) - Number(a.paymentFees || 0);
        ...
        const ap = Number(a.profitAfterAds ?? a.contributionMargin ?? 0);
        const bp = Number(b.profitAfterAds ?? b.contributionMargin ?? 0);
        return ap - bp;
    });
    return sorted.slice(0, limit).map((o) => ({
        ...
        profitAfterAdsAndShipping: o.profitAfterAdsAndShipping !== undefined ? round2(Number(o.profitAfterAdsAndShipping || 0)) : undefined,
    }));
}
