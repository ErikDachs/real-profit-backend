...
function assertScoreSortedDesc(items) {
...
describe("buildUnifiedOpportunityRanking", () => {
    ...
    it("limit: default 5, clamp 1..50", () => {
        ...
        const def = buildUnifiedOpportunityRanking(base);
        expect(def.top.length).toBeLessThanOrEqual(5);
        const lim1 = buildUnifiedOpportunityRanking({ ...base, limit: 1 });
        expect(lim1.top.length).toBe(1);
        const limTooHigh = buildUnifiedOpportunityRanking({ ...base, limit: 999 });
        // clamp to 50 but we only have 5 here anyway
        expect(limTooHigh.top.length).toBe(5);
        const limZero = buildUnifiedOpportunityRanking({ ...base, limit: 0 });
        // clamp min 1
