...
export function buildProfitScenarioResult(params) {
    ...
    const bPShipPct = baseline?.profitMarginAfterShippingPct === null ? null : Number(baseline?.profitMarginAfterShippingPct ?? 0);
    const sPShipPct = simulated?.profitMarginAfterShippingPct === null ? null : Number(simulated?.profitMarginAfterShippingPct ?? 0);
    const contributionMarginPctChange = bCMPct === null || sCMPct === null ? null : round2(Number(sCMPct) - Number(bCMPct));
    const breakEvenRoasChange = bBe === null || sBe === null ? null : round2(Number(sBe) - Number(bBe));
    const profitMarginAfterShippingPctChange = bPShipPct === null || sPShipPct === null ? null : round2(Number(sPShipPct) - Number(bPShipPct));
    return {
        baseline,
        simulated,
        delta: {
